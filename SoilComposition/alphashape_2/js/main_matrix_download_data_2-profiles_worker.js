...
onmessage = async function(e) {
    ...
    let worker_id = e.data[0]
    let combo = e.data[1]
    let alphahullContainer = e.data[2]


    await process(alphahullContainer).then(d=>{postMessage([worker_id, combo, d])})

    async function process(alphahullContainer){
        let results = {}
        ...
        // if (alphahullContainer[profiles[2]].length != 0 && intersections.length != 0 && intersections[0].length != 0) {
        ...
        results['intersection_area'] = intersection_area
        ...
        if (intersection_area === 0) {
            ...
            // alphahullContainer[Object.keys(alphahullContainer)[1]].forEach(d => {
            //     alphahullContainer[Object.keys(alphahullContainer)[2]].forEach(e => {
            //         d.forEach(d2 => {
            //             e.forEach(e2 => {
            //                 let dist = Math.sqrt(((d2.x - e2.x) ** 2) + ((d2.y - e2.y) ** 2))
            //                 dist < min_distance ? min_distance = dist : null
        ...
        results['separation'] = min_distance
        results['intersection_polygons'] = intersection_polygons

        if (area_calculation_error){
            results['intersection_area'] = 'error'
        }
        return results
    }
    ...
    //
    // let intersection_combos = e.data[5]
    //
    // let distance_object = {}
    ...
    // let _intersections = {}
    //
    // process(combinations, intersection_combos).then(()=> {
    //     let download_object = {}
    ...
    //                 // console.log(d,e)
    //                 if (intersection_combos.includes(d))
