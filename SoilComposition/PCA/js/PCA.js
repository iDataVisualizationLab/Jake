var PCA = function(){
    ...
    function mean(X){
        // mean by col
        var T = transpose(X);
        return T.map(function(row){ return d3v5.sum(row) / X.length; });
    }

    function transpose(X){
        return d3v5.range(X[0].length).map(function(i){
    ...
    function dot(X,Y){
        return X.map(function(row){
            return transpose(Y).map(function(col){
                return d3v5.sum(d3v5.zip(row,col).map(function(v){
                    return v[0]*v[1];
                }));
            });
        });
    ...
    function diag(X){
        return d3v5.range(X.length).map(function(i){
            return d3v5.range(X.length).map(function(j){ return (i === j) ? X[i] : 0; });
        });
    }

    function zeros(i,j){
        return d3v5.range(i).map(function(row){
            return d3v5.range(j).map(function(){ return 0; });
        });
    }
    ...
    function same(X,Y){
        return d3v5.zip(X,Y).map(function(v){
            return d3v5.zip(v[0],v[1]).map(function(w){ return w[0] === w[1]; });
        }).map(function(row){
    ...
    function sub(x,y){
        ...
        return d3v5.zip(x,y).map(function(v){
            if (typeof(v[0]) === 'number') return v[0]-v[1];
            else return d3v5.zip(v[0],v[1]).map(function(w){ return w[0]-w[1]; });
        });
    }

    function div(x,y){
        console.assert(x.length === y.length, 'dim(x) == dim(y)');
        return d3v5.zip(x,y).map(function(v){ return v[1]!==0 ? v[0]/(v[1]): 0; });
    ...
    function svd(A){
        ...
        var e = d3v5.range(n).map(function(){ return 0; });
        var q = d3v5.range(n).map(function(){ return 0; });
        var v = zeros(n,n);
