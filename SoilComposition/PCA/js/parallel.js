...
function buildParallelChart() {


    let dimObject = {};
    dims.forEach(dim => dimObject[dim.name] = dim);

    //** Causing bug when changing profiles**//
    // dims.forEach(d => {
    //     if(d.max == 0){
    //         d.hide = true
    ...
    //         //console.log(dimObject[soilPackages[packages[i]][j]+' Concentration'])
    //         if (document.getElementById(soilPackages[packages[i]][j]).checked == false) {
    //             dimObject[soilPackages[packages[i]][j]+' Concentration'].hide = true;
    //         }
    //         else if (document.getElementById(soilPackages[packages[i]][j]).checked == true) {
    //             //console.log(soilPackages[packages[i]][j]+' Concentration');
    //             dimObject[soilPackages[packages[i]][j]+' Concentration'].hide = false;
    //         }
    //
    //     }
    // }
...
// Load the data and visualization
    d3.csv("./data/" + prof + ".csv", function (raw_data) {
        ...
        data = raw_data.map(function (d) {
            for (var k in d) {
                // if (k.split(' ')[1] == "Concentration" && soilPackage.includes(k.split(' ')[0]) && !dimObject[k].hide) {
                if (k.split(' ')[1] == "Concentration" && !dimObject[k].hide) {
                ...
                .each(function (d) {
                    d3.select(this.parentNode)
                        ...
                        .style('stroke', function (d) {
                            return dims.find(({name}) => name === d).color
                        })
                    //console.log(d)
                });
            //console.log(ok);
...
// }
    ...
    function showSelected(selected) {
        ...
        p.select('circle').style("opacity", "0");
        ...
        p.each(function () {
            ...
            // else {d3.select(this).select('circle').style("fill", "black");}
                ...
                selected.forEach(d => {
                    ...
                    // else{ d3.select(this).select('circle').style("opacity", "0")}
