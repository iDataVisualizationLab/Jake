...
//
//     elementsList = await get_elements(profiles[0])
//     return elementsList;
// }
// getList().then(d => {
//     //console.log(d);
...
//     console.log (d[0])
//     elementsList.forEach(d=> console.log(d))
//
// });

function makeFeatureList(elements, profile){

    let featureList = ["Location", "Sample ID"];

    elements.forEach(d => {

        featureList.push(d+ " Concentration")

    });

    let notDetected = [ "Se Concentration", "Mo Concentration", "Ag Concentration", "Sb Concentration", "Bi Concentration" ]

    notDetected.forEach(d => {featureList = featureList.filter((f, index) => index !== featureList.indexOf(d));})

    //featureList = featureList.filter((f, index) => index !== featureList.indexOf("Se Concentration"));




    buildParallelChart(featureList, profile);

}

test = ['Ag Concentration','Al Concentration']
...
// // d3.csv("employment_perc_features4.csv", function(raw_data) {
// d3.csv("./data/R_normalized.csv", function(raw_data) {
//     // Convert quantitative scales to floats
//     data = raw_data.map(function(d) {
//         for (var k in d) {
//             // if (!_.isNaN(raw_data[0][k] - 0) && k != 'id') {
//             // if (!_.isNaN(raw_data[0][k] - 0) && k == 'Ag Concentration') {
//             if (!_.isNaN(raw_data[0][k] - 0) && test.includes(k)) {
...
function buildParallelChart(features, profile){
...
// Load the data and visualization
// d3.csv("employment_perc_features4.csv", function(raw_data) {
    d3.csv("./data/"+profile+".csv", function(raw_data) {
        // Convert quantitative scales to floats
        data = raw_data.map(function(d) {
            ...
            for (var k in d) {
                // if (!_.isNaN(raw_data[0][k] - 0) && k != 'id') {
                // if (!_.isNaN(raw_data[0][k] - 0) && k == 'Ag Concentration') {
                if (!_.isNaN(raw_data[0][k] - 0) && features.includes(k)) {
                    d[k] = parseFloat(d[k]) || 0;
                    //console.log(k);
                }
            };
...
function brush() {
    ...
    selected_states = [];
    selected.forEach(elem => selected_states.push(elem.state));
    //change_line_color();
...
function search(selection,str) {
    ...
    return _(selection).filter(function(d) { return pattern.exec(d.state); });
}
...
function showSelected(selected){
    ...
    selected.forEach(d => readings.push({L: d["Location"], D: d["Sample ID"]}));



    let p = d3.selectAll(".obs");
    ...
    p.each(function () {
        ...
        if (readings.length === 130){
            d3.select(this).select('circle').style("fill", "black");
        }

        //
        // else if (readings.includes(d3.select(this).data()[0]["Reading #"])){
        //     d3.select(this).select('circle').style("fill", "red");
        //     console.log(d3.select(this));
        // }
        //
        // else {d3.select(this).select('circle').style("fill", "black");}

        else{
            readings.forEach(d => {
                if (d3.select(this).data()[0]["Location"] === d.L && d3.select(this).data()[0]["Sample ID"] === d.D){
                    d3.select(this).select('circle').style("fill", "red");
                }
    ...
    // selected.forEach(d => {
    //     d3.selectAll('.obs').select('circle').filter((c, i) => i["Reading #"] === d["Reading #"]).style("fill", "red")
    // } )
    //
...
function change_line_color() {
    let p = d3.selectAll(".ids1");
    p.each(function () {
        if (selected_states.length === 50){
            d3.select(this).attr("stroke", "gray");
        }

        else if (selected_states.includes(d3.select(this).data()[0].id)){
            d3.select(this).attr("stroke", "red");
        }
        else{ d3.select(this).attr("stroke", "gray"); }
