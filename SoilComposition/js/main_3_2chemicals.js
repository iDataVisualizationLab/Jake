...
function buildVol1(){
    ...
    for ( let i = 0; i < particles; i ++ ) {
        ...
        // positions
        if ((valCa_t[i] >= valueMin && valCa_t[i] <= valueMax) || (valRb_t[i] >= valueMin && valRb_t[i] <= valueMax)) {
            if ((Math.pow(pos_x[i] - 50, 2) + Math.pow(pos_z[i] - 50, 2)) < (Math.pow(50, 2))) {
                ...
                if (valCa_t[i] >= valueMin && valCa_t[i] <= valueMax){
                    c1 = true;
                    //color1 = new THREE.Color(getColor1(valCa_t[i]));
                    //color.add(color1);
                }
                if (valRb_t[i] >= valueMin && valRb_t[i] <= valueMax){
                    c2 = true;
                    //color2 = new THREE.Color(getColor2(valRb_t[i]))
                    //color.add(color2);
                }
                ...
                if (c1 && c2){
                    color = new THREE.Color(getColor1(valCa_t[i]));
                    color2 = new THREE.Color(getColor2(valRb_t[i]));
                    color.add(color2);
                }
                else if (c1){
                    ...
                    color = new THREE.Color(getColor2(valRb_t[i]));
                }


                //color.lerpColors(low_color, high_color, val_t[i]);
                //let color = new THREE.Color(getColor1(valCa_t[i]));
                //let color = new THREE.Color(getColor2(valRb_t[i]));

                //let color2 = new THREE.Color(getColor2(valRb_t[i]))
