...
d3.csv("cpu1Temps_features.csv", function(raw_data) {
...
function brush() {
    ...
    selected_computes = [];
    selected.forEach(elem => selected_computes.push(elem.compute));
    change_line_color();
...
function search(selection,str) {
    ...
    return _(selection).filter(function(d) { return pattern.exec(d.compute); });
}
...
function render() {
    ...
    d3version5.select("div#line_chart").selectAll("svg").remove();

    const svg2 = d3version5.select("div#line_chart")
        .append("svg")
...
//----------------------------SCALES----------------------------//
            //const xScale = d3version5.scaleTime().range([0,width2]);
            const xScale = d3version5.scaleLinear().range([0,width2]);
            const yScale = d3version5.scaleLinear().rangeRound([height2, 0]);
            // xScale.domain(d3version5.extent(data, function(d){
            //      return (d.step)}));
            //xScale.domain(d3version5.extent(data, function(d){
            //    return timeConv(d.date)}));
            xScale.domain([0,1840]);
            yScale.domain([d3version5.min(slices, function (c) {
                return d3version5.min(c.values, function (d) {
                    return d.measurement - .025;
                });
            }), d3version5.max(slices, function (c) {
                return d3version5.max(c.values, function (d) {
                    return d.measurement + .025;
                });
            })
            ]);
...
//-----------------------------AXES-----------------------------//
            const yaxis = d3version5.axisLeft()
                //.ticks((slices[0].values).length)
                .ticks(Math.round((((slices[0].values).length)) / 80))
                //console.log(Math.round((((slices[0].values).length))/4))
                .scale(yScale);

            const xaxis = d3version5.axisBottom()
                //.ticks(10)
                //.tickFormat(d3version5.timeFormat('%b %d'))
                //.ticks(d3version5.timeDay.every(6000))
                .ticks(120)
                //.tickFormat(d3version5.timeFormat('%B %Y'))
                .scale(xScale);

//----------------------------LINES-----------------------------//
            const line = d3version5.line()
...
//----------------------------LINES-----------------------------//
            ...
            lines.append("path")
                ...
                //     }
                //     else {
                //         if (selected_states.includes(d.id)) {
                //             return 'red'
                //         } else {
                //             //console.log(d.id);
                //             return 'gray'
            ...
            //     // })
            //     .text(function (d) {
            //         if (selected_computes.length === 50) {
            //             return d.id;
            //         }
            //         else {
            //             if (selected_computes.includes(d.id)) {
            //                 return d.id;
            //             }
            //             // else {
            //             //     return 'gray'
...
function change_line_color() {
    ...
    p.each(function () {
        ...
        else if (selected_computes.includes(d3.select(this).data()[0].id)){
            d3.select(this).attr("stroke", highlight_color);
        }
        else{ d3.select(this).attr("stroke", path_color); }
