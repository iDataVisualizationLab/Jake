...
d3.csv("unemployment_feature_data.csv", function(raw_data) {
...
function brush() {
    ...
    selected_countries = [];
    selected.forEach(elem => selected_countries.push(elem.country));
    change_line_color();
...
function render() {
    ...
    d3version5.csv("unemployment_rank_data.csv").then(ranks => {
        d3version5.csv("unemployment_ts.csv").then(function (data) {
...
//----------------------------SCALES----------------------------//
            ...
            xScale.domain([1991,2020]);
            //xScale.domain(d3.extent(data, function (d) { return new Date(parseInt(d.year),0); }));
...
//----------------------------LINES-----------------------------//
            const line = d3version5.line().defined(d => d.measurement)
...
function change_line_color() {
    ...
    p.each(function () {
        ...
        else{ d3.select(this).attr("stroke", path_color); d3.select(this).attr("stroke-width", .75); d3.select(this).style("opacity", 0); }

    })

    let t = d3.selectAll(".serie_label");
    t.each(function () {
        if (selected_countries.length === 263){
